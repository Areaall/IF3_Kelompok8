...
if uploaded_file is not None:
    ...
    st.markdown("#### Contoh Data dari Sheet (Seluruh Baris):")
    st.dataframe(sheet, use_container_width=True)

    # Tampilkan versi pembulatan dari kolom terpilih
    if selected_columns:
        st.markdown("#### Tabel Versi Pembulatan dari Kolom Terpilih:")
        pembulatan_df = sheet[selected_columns].copy()
        for col in selected_columns:
            pembulatan_df[f"{col}_dibulatkan"] = np.round(pembulatan_df[col])

        st.dataframe(pembulatan_df, use_container_width=True)
    ...
    for col in selected_columns:
        ...
        simulasi_df = pd.DataFrame({
            "RNG": rng,
            "Simulasi Nilai": simulasi
        })

        # Hitung persentase kenaikan indeks dari nilai dasar 100
        simulasi_df["% Kenaikan Indeks"] = simulasi_df["Simulasi Nilai"] - 100
        ...
        simulasi_df["% Kenaikan Indeks (numeric)"] = simulasi_df["Simulasi Nilai"] - 100
        ...
        with col1:
            chart_data1 = simulasi_df[["Simulasi Nilai"]].reset_index().rename(columns={"index": "Urutan"})
            chart_data1["Urutan"] += 1
            chart_data1["Kategori"] = col
            st.write("Simulasi Nilai")
            line_chart1 = alt.Chart(chart_data1).mark_line().encode(
                x="Urutan",
                y=alt.Y("Simulasi Nilai",scale=alt.Scale(domain=[100, 150])),
                color="Kategori"
        ...
        with col2:
            ...
            st.write("Simulasi Persentase Kenaikan Indeks (berdasarkan 100)")
